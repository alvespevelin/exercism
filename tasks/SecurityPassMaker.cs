namespace exercism.tasks
{
    ...
    {
        public string GetDisplayNameT1(TeamSupport support) => (support is Staff) ? support.Title : "Too Important for a Security Pass";

        public string GetDisplayNameT2(TeamSupport support) => support is Staff ?
                ($"{support.Title}{(support is Security ? " Priority Personnel" : "")}")
                : "Too Important for a Security Pass";

        public string GetDisplayName(TeamSupport support) => support is Staff ?
                ($"{support.Title}{(support.GetType() == typeof(Security) ? " Priority Personnel" : "")}")
                : "Too Important for a Security Pass";
